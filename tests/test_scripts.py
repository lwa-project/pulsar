    ...
    def test_scripts(self):
        ...
        for script in _SCRIPTS:
            ...
            with self.subTest(script=name):
                ...
                pylint_args = [script, "-E", "--extension-pkg-allow-list=_psr,scipy.special", "--init-hook='import sys; sys.path=[%s]; sys.path.insert(0, \"%s\")'" % (",".join(['"%s"' % p for p in sys.path]), MODULE_BUILD)]
